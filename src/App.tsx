...
const Sidebar = ({ activeTab, setActiveTab, user, onSignOut }: { activeTab: string, setActiveTab: (tab: string) => void, user: User | null, onSignOut: () => void }) => {
  ...
  return (
    <div className="w-20 lg:w-64 bg-surface border-r border-slate-800 flex flex-col h-screen fixed left-0 top-0 z-50 transition-all">
      <div className="p-6 flex items-center gap-3">
        <div className="w-8 h-8 bg-gradient-to-br from-brand-green to-accent-blue rounded-lg flex items-center justify-center">
        ...
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-300 ${
              activeTab === item.id 
                ? 'bg-surface-hover text-brand-green' 
      ...
      <div className="p-6 flex flex-col gap-4 border-t border-slate-800">
        <div className="flex items-center gap-3">
          {user?.avatar ? (
            <img src={user.avatar} alt={user.name} className="w-8 h-8 rounded-full border border-slate-700" referrerPolicy="no-referrer" />
          ) : (
            <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-[10px] font-bold text-text-muted">
...
const PostComposer = ({ onPostCreated, token }: { onPostCreated: (post: Post) => void, token: string }) => {
  ...
  return (
    <div className="max-w-4xl space-y-12">
      <div className="bg-surface rounded-3xl p-8 border border-slate-800 shadow-sm">
        <h2 className="text-xl font-bold mb-8 flex items-center gap-2 text-text-primary">
          <Edit3 className="text-brand-green" />
          Compose New Post
        </h2>

        <div className="space-y-8">
          {/* AI Prompt Section */}
          <div className="bg-bg-dark/50 rounded-2xl p-6 border border-slate-800">
            ...
            <div className="flex gap-3">
              <input 
                ...
                className="flex-1 bg-bg-dark border border-slate-800 rounded-xl px-4 py-3 text-sm text-text-primary focus:outline-none focus:border-brand-green transition-all"
              />
              <button 
                onClick={generateAIContent}
                disabled={isGenerating || !prompt}
                className="bg-brand-green hover:bg-brand-green-dark disabled:opacity-50 text-white px-6 py-3 rounded-xl text-sm font-bold flex items-center gap-2 transition-all shadow-lg shadow-brand-green/10"
              >
          ...
          <textarea 
            ...
            className="w-full h-64 bg-bg-dark border border-slate-800 rounded-2xl p-6 text-text-primary focus:outline-none focus:border-brand-green resize-none transition-all font-light leading-relaxed"
          />
          ...
          <div className="flex flex-wrap gap-6 items-center">
            ...
            <div className="flex gap-3">
              {[
                ...
                { id: 'x', icon: Twitter, color: 'hover:text-white' },
                ...
                <button
                  ...
                  className={`p-4 rounded-2xl border transition-all ${
                    platforms.includes(p.id as Platform)
                      ...
                      : 'bg-bg-dark border-slate-800 text-text-muted ' + p.color
                  }`}
                >
                  <p.icon size={20} />
                </button>
          ...
          <div className="flex items-center gap-6 pt-8 border-t border-slate-800">
            ...
            <input 
              ...
              className="bg-bg-dark border border-slate-800 rounded-xl px-4 py-3 text-sm text-text-primary focus:outline-none focus:border-brand-green"
            />
            <div className="flex-1" />
            <button 
              onClick={handleCreate}
              disabled={!content || platforms.length === 0}
              className="bg-gradient-to-r from-brand-green to-accent-blue hover:opacity-90 disabled:opacity-50 text-white px-10 py-4 rounded-2xl font-bold shadow-xl shadow-brand-green/10 transition-all transform hover:-translate-y-0.5 active:translate-y-0"
            >
...
const Dashboard = ({ posts, token }: { posts: Post[], token: string }) => {
  ...
  const stats = [
    { label: 'Total Posts', value: dashboardStats.totalPosts, icon: Send, color: 'text-brand-green' },
    { label: 'Impressions', value: dashboardStats.totalImpressions.toLocaleString(), icon: Sparkles, color: 'text-accent-blue' },
    { label: 'Engagements', value: dashboardStats.totalEngagements.toLocaleString(), icon: CheckCircle2, color: 'text-brand-green' },
    { label: 'Eng. Rate', value: dashboardStats.avgEngagementRate, icon: Sparkles, color: 'text-brand-green' },
  ];
    ...
    <div className="space-y-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            {posts.slice(0, 4).map((post) => (
              <div key={post.id} className="group cursor-pointer border-b border-slate-800 pb-6 last:border-0">
                <div className="flex items-center gap-4 mb-3">
                  <div className="flex gap-1">
                    {post.platforms.map(p => (
                      <div key={p} className="text-text-muted group-hover:text-brand-green transition-colors">
        ...
        <div className="space-y-8">
          ...
          <div className="h-[240px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={[
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '8px', fontSize: '10px', color: '#f1f5f9' }}
          ...
          <div className="bg-surface p-6 rounded-2xl border border-slate-800">
            <p className="text-xs text-text-muted leading-relaxed">
              Your engagement is up <span className="text-brand-green">24%</span> compared to last week. 
  ...
  return (
    <div className="max-w-4xl space-y-12">
      ...
      <div className="grid grid-cols-1 gap-4">
        {accounts.map((acc) => (
          <div key={acc.id} className="bg-surface p-6 rounded-2xl border border-slate-800 flex items-center gap-6 hover:border-brand-green/30 transition-all group">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
              acc.connected ? 'bg-green-500/10 text-brand-green' : 'bg-slate-800 text-text-muted'
            }`}>
            ...
            <button 
              ...
              acc.connected 
                ? 'border border-slate-700 text-text-muted hover:text-red-500 hover:border-red-500/30' 
...
const AuthScreen = ({ onAuthSuccess, onBack }: { onAuthSuccess: (user: User, token: string) => void, onBack: () => void }) => {
  ...
  return (
    <div className="min-h-screen bg-bg-dark flex items-center justify-center p-6">
      <motion.div 
        ...
        className="max-w-md w-full bg-surface rounded-3xl shadow-xl border border-slate-800 p-10"
      >
        <div className="text-center mb-10">
          <div className="w-12 h-12 bg-gradient-to-br from-brand-green to-accent-blue rounded-xl flex items-center justify-center mx-auto mb-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {!isLogin && (
            <div>
              ...
              <input 
                ...
                className="w-full bg-bg-dark border border-slate-800 rounded-xl px-4 py-3 text-sm text-text-primary focus:outline-none focus:border-brand-green transition-all"
          ...
          <button 
            type="submit"
            disabled={isLoading}
            className="w-full bg-brand-green hover:bg-brand-green-dark disabled:opacity-50 text-white font-bold py-4 rounded-xl shadow-lg shadow-brand-green/10 transition-all mt-4"
          >
    ...
    <div className="min-h-screen bg-bg-dark">
      ...
      <nav className="flex items-center justify-between px-8 py-4 border-b border-slate-800">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-gradient-to-br from-brand-green to-accent-blue rounded-lg flex items-center justify-center shadow-sm">
            <Sparkles className="text-white w-5 h-5" />
          </div>
        ...
        <button 
          onClick={onLogin}
          className="bg-brand-green hover:bg-brand-green-dark text-white px-6 py-2 rounded-lg text-sm font-medium transition-all"
        >
          Log In
        </button>
      </nav>
      ...
      <section className="max-w-5xl mx-auto px-6 pt-24 pb-32 text-center">
        <h1 className="text-5xl md:text-7xl font-bold text-text-primary mb-6 tracking-tight">
          ...
          <span className="bg-gradient-to-r from-brand-green to-accent-blue bg-clip-text text-transparent">
            Automation Platform
          </span>
        </h1>
        <p className="text-text-muted text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed">
          Generate AI-powered content, schedule posts, and track engagement across your social media accounts. Powered by Gemini AI for lightning-fast content generation.
        </p>
        <button 
          onClick={onStart}
          className="bg-gradient-to-r from-brand-green to-accent-blue hover:opacity-90 text-white px-10 py-4 rounded-xl text-lg font-bold shadow-xl shadow-brand-green/20 transition-all transform hover:-translate-y-1"
        >
          Get Started Free
        </button>
      </section>
        ...
        {[
          {
            ...
            desc: 'Boost posts with Gemini AI. Generate content and choose your favorites.',
            icon: Sparkles,
          ...
          },
          {
            ...
            icon: BarChart,
            color: 'bg-brand-green-light/10 text-brand-green-light'
          }
        ].map((feature) => (
          <div key={feature.title} className="bg-surface p-8 rounded-3xl border border-slate-800 shadow-sm hover:shadow-md transition-all">
    ...
    <div className="space-y-16">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        {[
          ...
          <div key={stat.label} className="bg-surface p-6 rounded-2xl border border-slate-800">
      ...
      <div className="bg-surface rounded-3xl border border-slate-800 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-bg-dark/50 text-[10px] font-mono text-text-muted uppercase tracking-widest">
            ...
            <tbody className="divide-y divide-slate-800">
              {users.map((u) => (
                <tr key={u._id} className="hover:bg-bg-dark/30 transition-colors">
                  <td className="px-8 py-4 flex items-center gap-3">
                    {u.avatar ? <img src={u.avatar} className="w-8 h-8 rounded-full" referrerPolicy="no-referrer" /> : <div className="w-8 h-8 rounded-full bg-slate-800" />}
                    <span className="text-sm text-text-primary">{u.name}</span>
                  </td>
                  <td className="px-8 py-4 text-sm text-text-muted">{u.email}</td>
                  <td className="px-8 py-4">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${u.role === 'admin' ? 'bg-brand-green/10 text-brand-green' : 'bg-slate-800 text-text-muted'}`}>
                      {u.role}
                    </span>
                  </td>
      ...
      <main className="flex-1 ml-20 lg:ml-64 p-8 lg:p-12 max-w-7xl mx-auto w-full">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === 'settings' && (
              <div className="max-w-2xl bg-surface rounded-3xl p-8 border border-slate-800 shadow-sm">
                <h3 className="text-xl font-bold mb-8 text-text-primary">Account Settings</h3>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-5 bg-bg-dark rounded-2xl border border-slate-800">
                    <div>
